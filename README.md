![Repository Banner](/.github/assets/banner.png)

[![Arduino Code Quality](https://github.com/jurgenjacobsen/offline-wifi-feed/actions/workflows/arduino-integrity.yml/badge.svg)](https://github.com/jurgenjacobsen/offline-wifi-feed/actions/workflows/arduino-integrity.yml)
[![Web Integrity Check](https://github.com/jurgenjacobsen/offline-wifi-feed/actions/workflows/web-integrity.yml/badge.svg)](https://github.com/jurgenjacobsen/offline-wifi-feed/actions/workflows/web-integrity.yml)
[![wakatime](https://wakatime.com/badge/user/010adc07-6382-419f-87bc-0b3f507ee495/project/e8943d84-a442-48c2-9ce5-bfa9577436ec.svg)](https://wakatime.com/badge/user/010adc07-6382-419f-87bc-0b3f507ee495/project/e8943d84-a442-48c2-9ce5-bfa9577436ec)

# Offline WiFi Feed
An offline WiFi feed allowing users to access content without an internet connection. Made to be read where you have no internet, but want to read something on your phone.

## To Do
- [x] Setup basic functionality
- [x] Create a web interface and styling
- [x] Add first reading content
- [x] User view / connection count
- [ ] Add Setup Guide
- [ ] Automate Content Updates
 
## Sources
[The Electric Typewriter - Short Articles and Essays](https://tetw.org/Short_Articles_and_Essays)

## Content History
| Article / Book | Author        | Source                          | Shown Date       |
|----------------|---------------|---------------------------------|------------------|
| Crazy Love     | Steven Pinker | [tetw.org](https://tetw.org/)   | April 22th, 2026 |
|                |               |                                 |                  |
|                |               |                                 |                  |

## Inspiration
- [Levi Villarreal](https://leviv.cool/projects/2024/10/05/creating-a-pocket-wifi-portal.html)
- [Iffy Books Wifi Portal](https://github.com/iffybooks/pocket-wifi-portal)

## License
This project is licensed under the Apache License - see the [LICENSE](LICENSE) file for details.
